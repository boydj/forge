...
| Backups | none on Vultr (forge-backup + off-site below) | object storage for `forge-backup` archives, ~50 GB **~$5** (S3-compatible at ~$0.01-0.02/GB + minimums) | Vultr automatic backups +20% of plan ($1.00-1.20/instance); snapshots $0.05/GB-month (~$1.25 per 25 GB image) |
